    ...
    I2C: embedded_hal_async::i2c::I2c<Error = E>,
{
    ...
    pub async fn status(&mut self) -> u8 {
        let mut data = [0u8; 1];
        self.read_bytes(Registers::STATUS.to_u8(), &mut data).await;
        data[0]
    }
    pub async fn who_am_i(&mut self) -> u8 {
        self.read_register(Registers::WHO_AM_I.to_u8()).await
    }
    ...
    pub async fn set_odr(&mut self, rate: DataRate) {
        ...
        let _ = self
            .read_bytes(Registers::CTRL_REG1.to_u8(), &mut [register_state])
            .await;
    ...
    async fn set_active(&mut self) {
        let current_state = self.read_register(Registers::CTRL_REG1.to_u8()).await;
        let _ = self
            .set_register(Registers::CTRL_REG1.to_u8(), current_state | 0b00000010)
            .await;
        self.
    }
